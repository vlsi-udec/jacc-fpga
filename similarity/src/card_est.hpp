...
double hll_estimate (uint32_t sum_, uint16_t zeros)
{
	...
	// if (V > 0) {
	// 	double H = buckets * log((double)buckets / V);
	// 	if (H <= get_threshold(precision)) {
	// 		return H;
	// 	}
	// }

	// double sum = sum_ / double (1 << 16);
	...
	// double E = alpha * pow(buckets, 2.0) / sum;

	// if (E <= (5 * (double) buckets)) {
	// 	return E - estimate_bias (E, precision);
	// }
